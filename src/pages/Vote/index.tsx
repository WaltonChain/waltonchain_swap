  ...
  return (
    <PageWrapper gap="lg" justify="center">
      <DelegateModal
        ...
        title={showUnlockVoting ? t('Voter.UnlockVotes') : t('Voter.UpdateDelegation')}
      />
      <TopSection gap="md">
        <VoteCard>
          <CardBGImage />
          ...
          <CardSection>
            <AutoColumn gap="md">
              <RowBetween>
                <TYPE.white fontWeight={600}>{t('Voter.WSwapGovernance')}</TYPE.white>
              </RowBetween>
              <RowBetween>
                <TYPE.white fontSize={14}>{t('Voter.GovernanceDelegate')}</TYPE.white>
              </RowBetween>
              <ExternalLink
                ...
                target="_blank"
              >
                <TYPE.white fontSize={14}>{t('Voter.ReadGovernance')}</TYPE.white>
              </ExternalLink>
            </AutoColumn>
          </CardSection>
          <CardBGImage />
      ...
      <TopSection gap="2px">
        <WrapSmall>
          ...
          {showUnlockVoting ? (
            ...
            >
              {t('Voter.UnlockVoting')}
            </ButtonPrimary>
          ) : availableVotes && JSBI.notEqual(JSBI.BigInt(0), availableVotes?.raw) ? (
            <TYPE.body fontWeight={500} mr="6px">
              <FormattedCurrencyAmount currencyAmount={availableVotes} /> {t('Voter.Votes')}
            </TYPE.body>
          ) : uniBalance &&
            userDelegatee &&
            ...
            <TYPE.body fontWeight={500} mr="6px">
              <FormattedCurrencyAmount currencyAmount={uniBalance} /> {t('Voter.Votes')}
            </TYPE.body>
          ) : (
            ''
          )}
        ...
        {allProposals?.length === 0 && (
          <EmptyProposals>
            <TYPE.body style={{ marginBottom: '8px' }}>{t('Voter.NoFound')}</TYPE.body>
            <TYPE.subHeader>
              <i>{t('Voter.ProposalsAppear')}</i>
            </TYPE.subHeader>
          </EmptyProposals>
        )}
      ...
      <TYPE.subHeader color="text3">{t('Voter.ThreshholdProposals')}</TYPE.subHeader>
    </PageWrapper>
  )
}
