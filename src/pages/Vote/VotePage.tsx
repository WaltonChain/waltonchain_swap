  ...
  return (
    <PageWrapper gap="lg" justify="center">
      ...
      <DelegateModal
        ...
        title={t('Voter.VotePage.UnlockVotes')}
      />
      <ProposalInfo gap="lg" justify="start">
        <RowBetween style={{ width: '100%' }}>
          <ArrowWrapper to="/vote">
            <ArrowLeft size={20} /> {t('Voter.VotePage.AllProposals')}
          </ArrowWrapper>
        ...
        <AutoColumn gap="10px" style={{ width: '100%' }}>
          ...
          <RowBetween>
            <TYPE.main>
              {endDate && endDate < now
                ? t('Voter.VotePage.VotingEnded') + ' ' + (endDate && endDate.toLocaleString(DateTime.DATETIME_FULL))
                : proposalData
                ? t('Voter.VotePage.VotingApproximately') +
                  ' ' +
          ...
          {proposalData && proposalData.status === 'active' && !showVotingButtons && (
            <GreyCard>
              <TYPE.black>
                {t('Voter.VotePage.DelegatedBlock')} {proposalData.startBlock} {t('Voter.VotePage.EligibleVoting')}{' '}
                {showLinkForUnlock && (
                  <span>
                    <StyledInternalLink to="/vote">{t('Voter.VotePage.UnlockVoting')}</StyledInternalLink>{' '}
                    {t('Voter.VotePage.PrepareProposal')}
                  </span>
                )}
              </TYPE.black>
            </GreyCard>
        ...
        <CardWrapper>
          <StyledDataCard>
            <CardSection>
              <AutoColumn gap="md">
                <WrapSmall>
                  <TYPE.black fontWeight={600}>{t('Voter.VotePage.For')}</TYPE.black>
          ...
          <StyledDataCard>
            <CardSection>
              <AutoColumn gap="md">
                <WrapSmall>
                  <TYPE.black fontWeight={600}>{t('Voter.VotePage.Against')}</TYPE.black>
                  <TYPE.black fontWeight={600}>
        ...
        <AutoColumn gap="md">
          <TYPE.mediumHeader fontWeight={600}>{t('Voter.VotePage.Details')}</TYPE.mediumHeader>
        ...
        <AutoColumn gap="md">
          <TYPE.mediumHeader fontWeight={600}>{t('Voter.VotePage.Description')}</TYPE.mediumHeader>
          <MarkDownWrapper>
            <ReactMarkdown source={proposalData?.description} />
          </MarkDownWrapper>
        </AutoColumn>
        <AutoColumn gap="md">
          <TYPE.mediumHeader fontWeight={600}>{t('Voter.VotePage.Proposer')}</TYPE.mediumHeader>
          <ProposerAddressLink
