    ...
    <>
      ...
      <RowBetween my="1rem">
        <Text fontSize={16} fontWeight={500}>
          {t('MigrateV1.MigrateV1Exchange.Pooled')} {chainId && token.equals(WETH[chainId]) ? 'WETH' : token.symbol}:
        </Text>
        <RowFixed>
      ...
      <RowBetween mb="1rem">
        <Text fontSize={16} fontWeight={500}>
          {t('MigrateV1.MigrateV1Exchange.Pooled')} ETH:
        </Text>
        <RowFixed>
          <Text fontSize={16} fontWeight={500} marginLeft={'6px'}>
  ...
  return (
    <AutoColumn gap="20px">
      <TYPE.body my={9} style={{ fontWeight: 400 }}>
        {t('MigrateV1.MigrateV1Exchange.MigrateTrustless')}{' '}
        {chainId && (
          <ExternalLink href={getEtherscanLink(chainId, MIGRATOR_ADDRESS, 'address')}>
            <TYPE.blue display="inline">{t('MigrateV1.MigrateV1Exchange.MigrationContract')}</TYPE.blue>
          </ExternalLink>
        )}
        .
      </TYPE.body>
      ...
      {!isFirstLiquidityProvider && largePriceDifference ? (
        <YellowCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            {t('MigrateV1.MigrateV1Exchange.CorrectPrice')}
          </TYPE.body>
          <AutoColumn gap="8px">
            <RowBetween>
              <TYPE.body>V1 {t('MigrateV1.MigrateV1Exchange.Price')}:</TYPE.body>
              <TYPE.black>
            ...
            <RowBetween>
              <TYPE.body>V2 {t('MigrateV1.MigrateV1Exchange.Price')}:</TYPE.body>
              <TYPE.black>
            ...
            <RowBetween>
              <TYPE.body color="inherit">{t('MigrateV1.MigrateV1Exchange.PriceDifference')}</TYPE.body>
      ...
      {isFirstLiquidityProvider && (
        <PinkCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            {t('MigrateV1.MigrateV1Exchange.MigratedPool')}
          </TYPE.body>
      ...
      <LightCard>
        ...
        <div style={{ display: 'flex', marginTop: '1rem' }}>
          <AutoColumn gap="12px" style={{ flex: '1', marginRight: 12 }}>
            ...
            >
              {approval === ApprovalState.PENDING ? (
                <Dots>{t('MigrateV1.MigrateV1Exchange.Approving')}</Dots>
              ) : approval === ApprovalState.APPROVED ? (
                t('MigrateV1.MigrateV1Exchange.Approved')
              ) : (
                t('MigrateV1.MigrateV1Exchange.Approve')
              )}
            </ButtonConfirmed>
          </AutoColumn>
          <AutoColumn gap="12px" style={{ flex: '1' }}>
            ...
            >
              {isSuccessfullyMigrated ? (
                t('MigrateV1.MigrateV1Exchange.Success')
              ) : isMigrationPending ? (
                <Dots>{t('MigrateV1.MigrateV1Exchange.Migrating')}</Dots>
              ) : (
                t('MigrateV1.MigrateV1Exchange.Migrate')
              )}
            </ButtonConfirmed>
          </AutoColumn>
        </div>
      ...
      <TYPE.darkGray style={{ textAlign: 'center' }}>
        {t('MigrateV1.MigrateV1Exchange.YourWSwap')}
        {` ${token.symbol}/ETH `}
        {t('MigrateV1.MigrateV1Exchange.LiquidityV2')}
        {` ${token.symbol}/ETH`}
        {t('MigrateV1.MigrateV1Exchange.Liquidity')}
      </TYPE.darkGray>
    </AutoColumn>
  )
}
    ...
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        <AutoRow style={{ alignItems: 'center', justifyContent: 'space-between' }} gap="8px">
          ...
          <TYPE.mediumHeader>{t('MigrateV1.MigrateV1Exchange.MigrateLiquidity')}</TYPE.mediumHeader>
          <div>
            <QuestionHelper text={t('MigrateV1.MigrateV1Exchange.MigrateV2')} />
          </div>
        </AutoRow>

        {!account ? (
          <TYPE.largeHeader>{t('MigrateV1.MigrateV1Exchange.YouAccount')}</TYPE.largeHeader>
        ) : validatedAddress && chainId && token?.equals(WETH[chainId]) ? (
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
              {t('MigrateV1.MigrateV1Exchange.HoodMigrated')}
            </TYPE.body>
