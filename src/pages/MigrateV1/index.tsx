    ...
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        <AutoRow style={{ alignItems: 'center', justifyContent: 'space-between' }} gap="8px">
          <BackArrow to="/pool" />
          <TYPE.mediumHeader>{t('MigrateV1.MigrateLiquidity')}</TYPE.mediumHeader>
          <div>
            <QuestionHelper text={t('MigrateV1.MigrateV2')} />
          </div>
        </AutoRow>

        <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>{t('MigrateV1.BelowDeposit')}</TYPE.body>

        {!account ? (
          <LightCard padding="40px">
            <TYPE.body color={theme.text3} textAlign="center">
              {t('MigrateV1.ConnectLiquidity')}
            </TYPE.body>
          </LightCard>
        ) : isLoading ? (
          <LightCard padding="40px">
            <TYPE.body color={theme.text3} textAlign="center">
              <Dots>{t('MigrateV1.Loading')}</Dots>
            </TYPE.body>
          </LightCard>
        ) : (
          <>
            <AutoRow>
              <SearchInput
                ...
                placeholder={t('MigrateV1.EnterLiquidity')}
              />
            </AutoRow>
            {allV1PairsWithLiquidity?.length > 0 ? (
              <>{allV1PairsWithLiquidity}</>
            ) : (
              <EmptyState message={t('MigrateV1.NoFound')} />
            )}
          </>
        )}
      </AutoColumn>
