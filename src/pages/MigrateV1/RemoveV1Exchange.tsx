  ...
  return (
    <AutoColumn gap="20px">
      <TYPE.body my={9} style={{ fontWeight: 400 }}>
        {t('MigrateV1.RemoveV1Exchange.RemoveUnderlying')}
      </TYPE.body>
        ...
        <div style={{ display: 'flex', marginTop: '1rem' }}>
          ...
          >
            {isSuccessfullyRemoved ? (
              t('MigrateV1.RemoveV1Exchange.Success')
            ) : isRemovalPending ? (
              <Dots>{t('MigrateV1.RemoveV1Exchange.Removing')}</Dots>
            ) : (
              t('MigrateV1.RemoveV1Exchange.Remove')
            )}
          </ButtonConfirmed>
        </div>
      </LightCard>
      <TYPE.darkGray style={{ textAlign: 'center' }}>
        {t('MigrateV1.RemoveV1Exchange.YourV1')}
        {` ${chainId && token.equals(WETH[chainId]) ? 'WETH' : token.symbol}/ETH `}
        {t('MigrateV1.RemoveV1Exchange.LiquidityAssets')}
      </TYPE.darkGray>
    </AutoColumn>
  )
}
    ...
    <BodyWrapper style={{ padding: 24 }} id="remove-v1-exchange">
      <AutoColumn gap="16px">
        <AutoRow style={{ alignItems: 'center', justifyContent: 'space-between' }} gap="8px">
          ...
          <TYPE.mediumHeader>{t('MigrateV1.RemoveV1Exchange.RemoveLiquidity')}</TYPE.mediumHeader>
          <div>
            <QuestionHelper text={t('MigrateV1.RemoveV1Exchange.RemoveTokens')} />
          </div>
        </AutoRow>

        {!account ? (
          <TYPE.largeHeader>{t('MigrateV1.RemoveV1Exchange.YouAccount')}</TYPE.largeHeader>
          ...
          <EmptyState message={t('MigrateV1.RemoveV1Exchange.Loading')} />
        )}
      </AutoColumn>
    </BodyWrapper>
  )
