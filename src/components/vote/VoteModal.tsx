      ...
      {!attempting && !hash && (
        <ContentWrapper gap="lg">
          <AutoColumn gap="lg" justify="center">
            ...
            <TYPE.largeHeader>
              {availableVotes?.toSignificant(4)} {t('Vote.VoteModal.Votes')}
            </TYPE.largeHeader>
        ...
        <ConfirmOrLoadingWrapper>
          ...
          <AutoColumn gap="100px" justify={'center'}>
            <AutoColumn gap="12px" justify={'center'}>
              <TYPE.largeHeader>{t('Vote.VoteModal.SubmittingVote')}</TYPE.largeHeader>
            </AutoColumn>
            <TYPE.subHeader>{t('Vote.VoteModal.ConfirmWallet')}</TYPE.subHeader>
          </AutoColumn>
        </ConfirmOrLoadingWrapper>
      )}
      {hash && (
        <ConfirmOrLoadingWrapper>
          ...
          <AutoColumn gap="100px" justify={'center'}>
            <AutoColumn gap="12px" justify={'center'}>
              <TYPE.largeHeader>{t('Vote.VoteModal.TransactionSubmitted')}</TYPE.largeHeader>
            </AutoColumn>
            {chainId && (
              <ExternalLink href={getEtherscanLink(chainId, hash, 'transaction')} style={{ marginLeft: '4px' }}>
                <TYPE.subHeader>{t('Vote.VoteModal.ViewBrowser')}</TYPE.subHeader>
              </ExternalLink>
            )}
          </AutoColumn>
        </ConfirmOrLoadingWrapper>
